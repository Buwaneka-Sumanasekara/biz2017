...
public class C_ChequePayments {
    ...
    public TChqPayments getSpecificPayment(String ChqNo) throws Exception {
        String q = "SELECT CP.CHQ_NO AS CHQ_NO,CP.CHQ_DATE AS CHQ_DATE,CP.STATE AS STATE,CP.REFNO AS REFNO,CP.REFTRNTYP AS REFTRNTYP,CP.NOTE as CHK_NOTE,  ";
        ...
        if (rs.next()) {
            ...
            m.setNote(rs.getString("CHK_NOTE"));
        }
        return m;
    }
    ...
    public void UpdateState(TChqPayments chkPay, String State, Date date) throws Exception {
        ...
        if (!chkPay.getRefTrnTyp().equals("CRD_SET")) {
            ...
            //TODO: handle credit settlements
        }

    }

    public void CancelChq(TChqPayments chkPay) throws Exception {
        Map<String, String> m = new TreeMap<String, String>();
        m.put("STATE", "'D'");//delete
        m.put("NOTE", "'" + chkPay.getNote() + "'");
        m.put("MDDATE", "'" + sdf.format(new Date()) + "'");
        m.put("M_USER_MD", "'" + GLOBALDATA.GlobalData.CurUser.getId() + "'");
